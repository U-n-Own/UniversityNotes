...
We're gonna convolve our image with a *filter* that will set black everything that has high response to the image and white the other
...
img = np.random.randint(255, size=(512,512,3)).astype(np.uint8)
...
# Apply the Prewitt filters to the grayscale image
img_prewitt_x = cv2.filter2D(gray, cv2.CV_8U, kernel_x)
img_prewitt_y = cv2.filter2D(gray, cv2.CV_8U, kernel_y)
...
Of course blobs overlaps and we get *intrest points* then we proceed to center the *SIFT descriptor* at the center of our image.

Another thing is we need an heuristics that tells us which blobs matter for example a blob can be found in spatial vicinity, so 10 pixel that are nearby can indicate a blob, but also with different scaling over a certain pixel this because we could get too many responses, how do we do? Typically we suppress everything on spatial scale give a certain sigma, so if two pixel are near we pick the highest one same with different scales on the same pixel
...
The idea is to extract *covariant regions* of an image such that if you change things they tend to stay togheter, if we perturbe an image we want that certain points are stable to mutation on the image
- 
...
***Thresholding*** so that part of image becomes black and part white, with different thresholds and check what parts (these need to be *connected*) that's important.
...
We want to calculate minimal change of connected components.

![[Pasted image 20230611192634.png]]
...
Partitioning an image into set of homogeneous pixels, hoping to match objects or subparts, images contains segments, that are area with a certain continuity
...
Reasoning in the *Spectral Domain*, a good way to find an orthonormal basis is to find the eigenvectors corresponding to the smallest eigenvalue, picking the **Laplacian** (can trasform adjacency matrix) of the graph, find the eigenvectors and break in two graphs, then repeat on the two sub-graph until we get a stable point. This is the basic idea.

$$
L_{aplacian} = D_{egree} - A_{djacency}
$$

Where the D has only the diagonal part full of how many connection there are for that node, while A describe with what nodes is connected.

Why Laplacian? This matrix characterize **connectivity** in our graph. The off-diagonal elements of the Laplacian matrix indicate the strength of connections between nodes, while the diagonal elements represent the degree of each node. This matrix is used as criterion for apply NCut.
